...
describe(ConceptMatchService, () => {
    ...
    describe('simple matches', () => {
        it('should not match anything is query is empty', () => {
            ...
            const matches: ConceptGraph[] = matcher.getMatches(query, data, { shouldIncludeQueryInResult: true })

            expect(matches).toEqual([])
        })

        fit('should match all concepts if asking for everything', () => {
            const data: ConceptGraph = ConceptGraph.fromModel({
                'sky': { '-is->': 'blue' }
            })
            const query: ConceptGraph = ConceptGraph.fromModel({
            ...
            console.log('GOT N MATCHES:', matches.length)

            const expectedAnswer: ConceptGraph = ConceptGraph.fromModel({
                'sky': { '-is->': 'blue' }
            })
            expect(matches).not.toEqual([])
            expect(matches[0].toModel()).toEqual(expectedAnswer.toModel())
        })
